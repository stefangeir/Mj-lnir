...
class FacebookTVDatasource: NSObject, UITableViewDataSource
{
    ...
    var textBoxWidthHasBeenSet = false
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if indexPath.row < fbPosts.count {
            ...
            if !textBoxWidthHasBeenSet {
                textBoxWidth = cell.fbPostTitleLabel.bounds.size.width
                textBoxWidthHasBeenSet = true
            }
    ...
    func parseData() {
        ...
        for i = indexOfNextPostToParse; i < datamodel.jsonDataArray.count; i++ {
            ...
            thisPost.PictureUrl = post[FBGraphStrings.PictureURL].string
            thisPost.ID = post[FBGraphStrings.Id].string
            
            if let postLinkName = post[FBGraphStrings.LinkName].string {
                var postTitle = postLinkName
                
                if let caption = post[FBGraphStrings.Caption].string {
                    postTitle += " - \(caption)"
                }
                thisPost.Title = postTitle
            } else if let postsStory = post[FBGraphStrings.Story].string {
                thisPost.Title = postsStory
            }
            
            if let message = post[FBGraphStrings.Message].string {
                thisPost.Message = message
            }
            
            if let created = post[FBGraphStrings.CreatedTime].string {
                let date = NSDate(fromInternetDateTimeString: created, formatHint: DateFormatHintRFC3339)
                thisPost.CreatedTime = getDateString(date)
            }
...
//                    
    ...
    func handleFBError(note: NSNotification) {
        ...
        if let errorDict = note.userInfo as? [String:NSError] {
            if let error = errorDict[FacebookNetworking.UserInfoKey] {
